...
for (window_split in test_window_range) {
    ...
    # moving_test_window_dat <- paste("moving_test_window/30_dat_",window_split,".Rds", sep = "")
    ...
    cat("original model sum of date differences: ", sum(sanitarian_dat[, date_difference]), "\n")
    cat("original model mean of date differences: ", mean(sanitarian_dat[, date_difference]), "\n")
    cat("original model num of rows with violation: ", nrow(sanitarian_dat), "\n\n")
    ...
    if (absolute_mode) {
        ...
        cat("default ordering sum of date differences: ", sum(sanitarian_dat[, date_difference]), "\n")
        cat("default ordering  mean of date differences: ", mean(sanitarian_dat[, date_difference]), "\n")
        cat("default ordering  num of rows with violation: ", nrow(sanitarian_dat), "\n\n")
    ...
    # write.csv(newdf, paste("moving_test_window/",window_split,"_window_inspection_time_geo.csv", sep = ""), row.names=FALSE)
    # write.csv(newdf, paste("no_sanitarians_data/ns_",window_split,"_window_inspection_time_geo.csv", sep = ""), row.names=FALSE)
    # write.csv(newdf, paste("cluster_models/",window_split,"_window_inspection_time.csv", sep = ""), row.names=FALSE)
    write.csv(newdf, paste("suppressed_sanitarian_features/",window_split,"_window_inspection_time_geo.csv", sep = ""), row.names=FALSE)
