#Learn how to use the Yelp Fusion API with the documentation
...
library(yelpr)
apikey <- "THJor0XATJ5OZeP89_qLxIanMHZ-reEbNRIMScZ92qB1VJQ9tWOtxZ5eNj0YAWFZGVGDjkPVkCdBIwTsqfb7Z2eZUuw-m83JCI8kRxnFau1YMYdxyVpGheThi50nX3Yx"

#Client ID
client <- "yyMsz2PJ_URN_OTO8l5kAw"

#API Key
#THJor0XATJ5OZeP89_qLxIanMHZ-reEbNRIMScZ92qB1VJQ9tWOtxZ5eNj0YAWFZGVGDjkPVkCdBIwTsqfb7Z2eZUuw-m83JCI8kRxnFau1YMYdxyVpGheThi50nX3Yx
...
dat[ , Test := Inspection_Date >= (max(Inspection_Date) - 90)] #90
datTest <- dat[Test == TRUE]

newDat <- datTest[, c("AKA_Name", "Address", "Zip")]
#newDat$ID <- seq.int(nrow(newDat))
print(colnames(newDat))
print(newDat$AKA_Name[102])
print(newDat$Address[102])
print(newDat$Zip[102])

#print(business_match_name(apikey, type = "best match", name = newDat$AKA_Name[2], address1 = newDat$Address[2], city = "Chicago", state = "IL", country = "US"))
response <- business_search(apikey, term = newDat$AKA_Name[1], location = newDat$Address[1], limit = 1)$business$price
print(response)
print(nchar(response))

price <- function(x){
    response <- business_search(apikey, term = x[1], location = x[2], limit = 1)$business$price
    print(x[1])
    print(x[2])
    print(x[3])
    print(response)
    if(is.null(response)){
        ...
        return(nchar(response))
    }
}

newDat$Price <- apply(newDat, 1 , price)
newDat
# search businesses with keyword 'chinese' in 'New York'

# print((business_ny$businesses)$price)
