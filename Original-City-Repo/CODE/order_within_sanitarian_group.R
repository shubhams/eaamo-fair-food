...
for (window_split in test_window_range) {
    ...
    l <- 1
    for (location in locationDivisions) {
        row <- c("all location", mean(sanitarian_dat[criticalFound == 1, date_difference]), 
                 nrow(sanitarian_dat[criticalFound == 1]))
        print_location_violation_time(sanitarian_dat, locationNames[l], location, row)
        l <- l + 1
    }
    ...
    if (absolute_mode) {
        ...
        l <- 1
        for (location in locationDivisions) {
            row <- c("default location", mean(sanitarian_dat[criticalFound == 1, date_difference]), 
                     nrow(sanitarian_dat[criticalFound == 1]))
            print_location_violation_time(sanitarian_dat, locationNames[l], location, row)
            l <- l + 1
        }
    ...
    # write.csv(newdf, paste("cluster_models/",window_split,"_window_geo.csv", sep = ""), row.names=FALSE)
    write.csv(newdf, paste("moving_test_window/",window_split,"_window_geo.csv", sep = ""), row.names=FALSE)
    # write.csv(newdf, paste("no_sanitarians_data/ns_",window_split,"_window_geo.csv", sep = ""), row.names=FALSE)
    # write.csv(newdf, paste("suppressed_sanitarian_features/",window_split,"_window_geo.csv", sep = ""), row.names=FALSE)
}
