...
print("\n Accuracy table for different algorithms based on race + gender")
print("------------------------")
...
for race in races:
    for gender in genders:
        tags.append(race + ',' + gender)
        is_race = train_data['race'] == race
        race_data = train_data[is_race]
        race_label = train_label[is_race]
        
        is_gender = race_data['gender'] == gender
        data = race_data[is_gender]
        label = race_label[is_gender]
        fraction = len(label) / total_size
...
table = pd.DataFrame(info, columns = cols, index = tags)
print(table)
