...
import crime_preprocess as cp

#preprocess train and test data to get data without sensitive labels
train_data, train_label, test_data, test_label = cp.crime_preprocess('communities_data.txt', 'communities_test.txt')

total_size = len(train_label)

train_data_no_sensitive = train_data.drop(columns = ['rB', 'rW', 'rA', 'rH'])
test_data_no_sensitive = test_data.drop(columns = ['rB', 'rW', 'rA', 'rH'])

#transform data into one hot vectors
train_data_1hot = cp.get_1hot(train_data_no_sensitive)
test_data_1hot = cp.get_1hot(test_data_no_sensitive)
...
greedy_overall_acc = cp.accuracy_randomized(classifiers, weights, train_data_scaled, train_label)
...
seqpav_overall_acc = cp.accuracy_randomized(seqpav, seqpav_weights, train_data_scaled, train_label)
...
for race in races:
    ...
    data_no_sensitive = data.drop(columns = ['rB', 'rW', 'rA', 'rH'])
    data_1hot = cp.get_1hot(data_no_sensitive)
    data_scaled = scaler.transform(data_1hot)

    erm_acc = classifiers[0].score(data_scaled, label)
    greedy_acc = cp.accuracy_randomized(classifiers, weights, data_scaled, label)

    clf = Mymodel(random_state = 0, solver = 'newton-cg')
    clf.fit(data_scaled, label)
    opt_acc = clf.score(data_scaled, label)

    seqpav_acc = cp.accuracy_randomized(seqpav, seqpav_weights, data_scaled, label)
