import os,sys
import numpy as np
from prepare_taiwan_lending_data import *
...
def test_lending_data():
	...
	ut.compute_p_rule(x_control['x2'], y) # compute the p-rule in the original data
	...
	for i, (train_index, test_index) in enumerate(sss.split(X, y)):
		...
		sex_col_idx = og_cols.index('x2')+1
		...
		def train_test_classifier():
			...
			fairness_scores_dict['demographic_parity_difference'].append(demographic_parity_difference(fair_y_true, fair_y_pred, sensitive_features=x_control_test['x2']))
			fairness_scores_dict['equal_opportunity_difference'].append(MetricFrame(metrics=true_positive_rate, y_true=fair_y_true, y_pred=fair_y_pred, sensitive_features=x_control_test['x2']).difference())
		...
		sensitive_attrs_to_cov_thresh = {'x2': allowed_thresh}
		print()
		...
		np.savetxt(f"../../lending_scores/zafar_scores_taiwan_{allowed_thresh}_fold_{i}.csv", list_to_write, delimiter=",")
	#fairness stats
