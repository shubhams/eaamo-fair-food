...
def train_model(x, y, x_control, loss_function, apply_fairness_constraints, apply_accuracy_constraint, sep_constraint, sensitive_attrs, sensitive_attrs_to_cov_thresh, gamma=None):
    ...
    if apply_accuracy_constraint == 0: #its not the reverse problem, just train w with cross cov constraints
        ...
        w = minimize(fun = loss_function,
            ...
            method = 'SLSQP',
            # method = 'COBYLA',
            # bounds=[(-1.0, 1.0) for i in range(x.shape[1])],
...
def print_classifier_fairness_stats(acc_arr, correlation_dict_arr, cov_dict_arr, s_attr_name):
    ...
    non_prot_pos = correlation_dict[s_attr_name][1].get(1, 0)
    prot_pos = correlation_dict[s_attr_name][0].get(1, 0)
    p_rule = (prot_pos / non_prot_pos) * 100.0 if non_prot_pos != 0 else 0
...
def check_accuracy(model, x_train, y_train, x_test, y_test, y_train_predicted, y_test_predicted):
    ...
    def get_accuracy(y, Y_predicted):
        ...
        accuracy = float(sum(correct_answers)) / float(len(correct_answers)) if float(len(correct_answers)) != 0.0 else 0.0
...
def split_food(x_all, y_all, x_control_all, test_idx):

    mask_train = np.where(test_idx == False)[0]
    mask_test = np.where(test_idx == True)[0]
    x_all_train = x_all[mask_train]
    x_all_test = x_all[mask_test]
    y_all_train = y_all[mask_train]
    y_all_test = y_all[mask_test]
    x_control_all_train = {}
    x_control_all_test = {}
    for k in list(x_control_all.keys()):
        x_control_all_train[k] = x_control_all[k][mask_train]
        x_control_all_test[k] = x_control_all[k][mask_test]
...
def get_classification_metrics(y_predicted, y_test):
    print(f"confusion matrix: \n{confusion_matrix(y_test, y_predicted)}")
    print(f"precision score: \n{precision_score(y_test, y_predicted, average='binary', pos_label=1.0)}")
    return precision_recall_fscore_support(y_test, y_predicted, pos_label=1.0, average='binary')

def get_fairness_metric(y_predicted, x_control_test, y_test, sensitive_attrs, criteria = 'dp'):
    """_summary_

    Args:
        y_predicted (_type_): _description_
        x_control_test (_type_): _description_
        y_test (_type_): _description_
        sensitive_attrs (_type_): _description_
        criteria (str, optional): _description_. Defaults to 'dp'.
    """
    for attr in sensitive_attrs:

        attr_val = []
        for v in x_control_test[attr]: attr_val.append(v)
        assert(len(attr_val) == len(y_predicted) == len(y_test))

        df_dict = {"Y_hat":y_predicted, "Y":y_test, "A":attr_val}
        df = pd.DataFrame(df_dict)

        if criteria == 'dp':
            # demographic parity
            result_df = df
            message = "P(Y^=1.0 | A={group}) = {frac}"
        elif criteria == 'eopp':
            # equal opportunity
            result_df = df[df["Y"]==1.0]
            message = "P(Y^=1.0 | A={group}, Y=1.0) = {frac}"
        
        fair_prob = result_df[result_df["Y_hat"]==1.0]["Y_hat"].count()/result_df["Y_hat"].count()
        print(f"abs fair prob: {fair_prob}")

        fairness_violation = list()
        for group in np.unique(attr_val):
            denom = result_df[result_df["A"]==group]["A"].count()
            numer = result_df[(result_df["Y_hat"]==1.0) & (result_df["A"]==group)]["A"].count()
            group_fair_prob = numer/denom
            print(message.format(group=group, frac=group_fair_prob))
            fairness_violation.append(abs(fair_prob - group_fair_prob))
        print(f"{criteria} violation: {np.sum(fairness_violation)}")
    return np.sum(fairness_violation)