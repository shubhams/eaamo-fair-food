...
def prepare_food(window, majority):
    filePath = 'datasets/A,Y,X/Food_inspection/'

#     dataA = pd.read_csv(f"{filePath}Food_inspection_A_{window}_{color}.csv")
    dataA = pd.read_csv(f"{filePath}Food_inspection_A_{window}_{majority}.csv")
    dataY = pd.read_csv(f"{filePath}Food_inspection_Y_{window}_{majority}.csv")
    dataX = pd.read_csv(f"{filePath}Food_inspection_X_{window}_{majority}.csv")
#     perm = np.genfromtxt(filePath + 'Food_perm.csv', delimiter=',')
